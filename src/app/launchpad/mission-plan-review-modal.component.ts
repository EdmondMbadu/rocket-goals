...
Return ONLY a JSON array of objects with keys: "dayNumber" (number) and "title" (string). No extra text.
  ...
  printPlan() {
    ...
    const rows = this.milestones()
      .map(item => `
        <div class="row">
          <div class="day-badge">Day ${item.dayNumber}</div>
          <div class="row-body">
            <div class="row-title">${this.escapeHtml(item.title)}</div>
            <div class="row-meta">Milestone ${item.dayNumber} of ${this.milestones().length}</div>
          </div>
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            body { font-family: 'Inter', 'Helvetica Neue', Arial, sans-serif; padding: 40px; color: #0f172a; background: #f8fafc; }
            .page { max-width: 900px; margin: 0 auto; background: #ffffff; border-radius: 24px; padding: 36px 40px 44px; border: 1px solid #e2e8f0; }
            .header { display: flex; justify-content: space-between; align-items: center; gap: 20px; margin-bottom: 24px; }
            .title-block h1 { font-size: 30px; margin: 0 0 6px; letter-spacing: -0.02em; }
            .subtitle { color: #475569; margin: 0; font-size: 14px; }
            .meta { text-align: right; color: #64748b; font-size: 12px; }
            .meta .badge { display: inline-block; padding: 6px 12px; border-radius: 999px; border: 1px solid #fecaca; background: #fff1f2; color: #b91c1c; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; font-size: 10px; margin-bottom: 6px; }
            .summary { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 24px 0 28px; }
            .summary-card { border: 1px solid #e2e8f0; border-radius: 16px; padding: 16px; background: #f8fafc; }
            .summary-label { color: #64748b; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 6px; }
            .summary-value { font-size: 18px; font-weight: 700; color: #0f172a; }
            .rows { display: flex; flex-direction: column; gap: 10px; }
            .row { display: grid; grid-template-columns: auto 1fr auto; gap: 16px; padding: 14px 16px; border-radius: 16px; border: 1px solid #e2e8f0; background: #ffffff; align-items: center; }
            .day-badge { padding: 6px 12px; border-radius: 999px; border: 1px solid #fecaca; background: #fff1f2; color: #b91c1c; font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: 0.12em; }
            .row-title { font-size: 15px; font-weight: 600; color: #0f172a; margin-bottom: 4px; }
            .row-meta { font-size: 12px; color: #64748b; }
            .row-check { width: 18px; height: 18px; border-radius: 4px; border: 2px solid #cbd5f5; }
            .footer-note { margin-top: 28px; font-size: 12px; color: #94a3b8; text-align: center; }
            @media print {
          ...
          <div class="page">
            <div class="header">
              <div class="title-block">
                <h1>${title}</h1>
                <p class="subtitle">A clear daily roadmap for your RocketGoal mission.</p>
              </div>
              <div class="meta">
                <div class="badge">Mission Plan</div>
            ...
            <div class="summary">
              <div class="summary-card">
                ...
                <div class="summary-value">${this.milestones().length}</div>
              </div>
              <div class="summary-card">
                <div class="summary-label">Goal Focus</div>
                <div class="summary-value">${this.escapeHtml(this.goalTitle())}</div>
              </div>
              <div class="summary-card">
                <div class="summary-label">Status</div>
                <div class="summary-value">Ready to Launch</div>
              </div>
            </div>
            <div class="rows">
              ${rows}
            </div>
            <div class="footer-note">Generated by RocketGoals · Keep this plan visible and celebrate each milestone.</div>
          </div>
        </body>
      </html>
    `);
